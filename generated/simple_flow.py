...
# --- Mock Customer Data & Internal Function ---
MOCK_CUSTOMER_DB = {
...
def get_customer_info(account_id: str) -> dict | None:
    """Internal function to fetch full customer data dict."""
...
def customer_lookup_tool(account_id: str) -> str:
    ...
    customer_data = get_customer_info(account_id) # Use internal function
    if customer_data:
...
llm = ChatGoogleGenerativeAI(model="gemini-1.5-pro-latest", temperature=0.2) # Lower temp for more predictable responses
# LLM bound with the lookup tool
llm_with_customer_tool = llm.bind_tools([customer_lookup_tool])
print("Gemini LLMs Initialized.")

# --- Agent State Definition ---
class AgentState(TypedDict):
    messages: Annotated[List[AnyMessage], operator.add]
    user_info: dict | None # Stores the full dict after successful lookup

# --- Agent Node: Customer Interaction & Verification ---
def customer_interaction_node(state: AgentState) -> dict:
    ...
    if isinstance(last_message, ToolMessage) and last_message.name == 'customer_lookup_tool':
        ...
        account_id = None
        # Find the original tool call args (account_id)
        ...
        retrieved_data = get_customer_info(account_id) if account_id else None # Get full data dict
...
You have successfully retrieved the customer's information.
...
Do not ask further questions in this response.
"""
...
Tool Result: {tool_result_content}
Inform the user clearly that the account ID was not found and ask them to provide a valid one if they have it.
Example: "Sorry, I couldn't find an account with the ID '{account_id or 'provided'}'. Please provide a valid account ID."
"""
            ai_response = llm.invoke(failure_prompt)
            # Update state (ensure user_info is None) and messages
            return {"user_info": None, "messages": [ai_response]}

    # --- Scenario 2: Handle Human Input / Standard Interaction ---
    elif isinstance(last_message, HumanMessage):
        if user_info:
            # Info already known - confirm it as requested
            ...
            # Info not known - analyze user message
...
1.  If the user explicitly provided something that looks like an Account ID (e.g., numbers, maybe letters/numbers mix), call the `customer_lookup_tool` with that ID.
...
Respond conversationally OR call the tool based on the analysis.
"""
            # Use the LLM with the tool bound
    ...
    # This might happen if the last message was AI (e.g., the initial greeting)
    else:
         print("--- No specific action triggered (e.g., initial state or unexpected AI message). Passing turn. ---")
         # Add no new messages, just return current state essentially
         # Returning empty dict might cause issues if it's the entry point.
         # Let's ensure messages are always returned.
         return {"messages": []} # Adds nothing, effectively waits.
...
# --- Graph Definition ---
memory = SqliteSaver.from_conn_string(":memory:") # Use DB file for persistence between runs if needed
...
def should_continue(state: AgentState) -> str:
    ...
    if isinstance(last_message, AIMessage) and last_message.tool_calls:
        # If AI requested a tool, execute it
        return "execute_tool"
    else:
        # In all other cases for this simplified graph (AI asked for ID, AI confirmed info,
        # AI reported failure, general chat response), stop the current run.
        return END
...
# After executing the tool, always go back to the interaction node to process the result
workflow.add_edge("execute_tool", "interaction")

# Compile the graph
app = workflow.compile(checkpointer=memory)
print("LangGraph Compiled (Simplified Verification Agent).")

# --- Running the Graph ---
def run_conversation():
    thread_id = f"isp-verify-thread-{os.urandom(4).hex()}" # New thread for each run
    config = {"configurable": {"thread_id": thread_id}}
    print(f"\n--- Starting Conversation (Thread: {thread_id}) ---")

    # Initial greeting (optional, can be done by agent too)
    print("AI: Hello! I'm the Horizon Broadband verification assistant. How can I help you today? If you need account specific help, please provide your Account ID.")
    ...
    while True:
        ...
        try:
            # Invoke the graph. It will run until it hits END or needs tool execution.
            response = app.invoke(inputs, config=config)

            # Extract the last AI message to display
            final_ai_message = ""
            if response and response.get('messages'):
                 # Find the last AIMessage added in the *latest* step
                 for msg in reversed(response['messages']):
                     # Check if it's an AIMessage AND it's likely the final output
                     # Note: In invoke, 'response' is the final state. The last message overall might be Human or Tool.
                     # We need to find the *last AI response* generated.
                     if isinstance(msg, AIMessage):
                          final_ai_message = msg.content
                          break # Found the last AI output

            if final_ai_message:
                 print(f"AI: {final_ai_message}")
            else:
                 # This occurs if the graph ended without the interaction node producing a final AIMessage
                 # (e.g., if should_continue went straight to END after a human message that didn't trigger tool/response)
                 print("AI: (Waiting for next input or process ended)")

            # Check if the graph is finished (for this turn) - more complex to check exact END state after invoke
            # If the last response didn't ask a question or call a tool, assume the turn is over.
